...
public abstract class SharedDeliverySystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<DeliveryComponent, DestructionEventArgs>(OnDestruction);
        SubscribeLocalEvent<DeliveryComponent, DamageChangedEvent>(OnDamage);
        SubscribeLocalEvent<DeliveryComponent, BreakageEventArgs>(OnBreak);
    }
    ...
    private void OnExamine(Entity<DeliveryComponent> ent, ref ExaminedEvent args)
    {
        ...
        if (ent.Comp.IsFragile)
            args.PushMarkup(Loc.GetString("delivery-fragile-examine"));

        if (ent.Comp.IsPriority)
            args.PushMarkup(Loc.GetString(ent.Comp.IsProfitable ? "delivery-priority-examine" : "delivery-priority-inactive-examine"));
    ...
    protected virtual void OnDestruction(Entity<DeliveryComponent> ent, ref DestructionEventArgs args)
    {
        ...
        {
            _container.AttachParentToContainerOrGrid((entity, Transform(entity)));
        }
    ...
    protected virtual void OnBreak(Entity<DeliveryComponent> ent, ref BreakageEventArgs args)
    {
        _appearance.SetData(ent, DeliveryVisuals.IsBroken, true);
    }
    ...
    protected virtual void OpenDelivery(Entity<DeliveryComponent> ent, EntityUid user, bool attemptPickup = true)
    {
        var deliveryName = _nameModifier.GetBaseName(ent.Owner);
    ...
    protected virtual void WithdrawSpesoPenalty(Entity<DeliveryComponent?> ent) { }
}
