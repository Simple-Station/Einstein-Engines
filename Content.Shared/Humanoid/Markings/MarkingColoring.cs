...
public static class MarkingColoring
{
    ...
    )
    {
        ...
        var defaultColor = prototype.Coloring.Default.GetColor(skinColor, markingSet);
        ...
        else
        {
            ...
            for (var i = 0; i < prototype.Sprites.Count; i++)
            {
                ...
                {
                    var marking_color = layerColoring.GetColor(skinColor, markingSet);
...
public sealed partial class LayerColoringDefinition
{
    ...
    public Color GetColor(Color? skin, MarkingSet markingSet)
    {
        var color = Type.GetColor(skin, markingSet);
        if (color == null)
        {
            foreach (var type in FallbackTypes)
            {
                color = type.GetColor(skin, markingSet);
                if (color != null) break;
            }
        }
...
public abstract partial class LayerColoringType
{
    ...
    public abstract Color? GetCleanColor(Color? skin, MarkingSet markingSet);
    public Color? GetColor(Color? skin, MarkingSet markingSet)
    {
        var color = GetCleanColor(skin, markingSet);
        // Negative color
