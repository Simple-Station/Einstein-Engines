...
public sealed class ParticleAcceleratorControlMenu : BaseWindow
{
    ...
    public ParticleAcceleratorControlMenu(ParticleAcceleratorBoundUserInterface owner)
    {
        ...
        _stateSpinBox = new() { Value = 0, IsValid = StrengthSpinBoxValid, };
                                ...
                                Children =
                                {
                                    ...
                                    new BoxContainer
                                    {
                                        ...
                                        Children =
                                        {
                                            ...
                                            {
                                                Text = Loc.GetString(
                                                    "particle-accelerator-control-menu-strength-label") + " ",
                                                HorizontalExpand = true,
                                                HorizontalAlignment = HAlignment.Left
                                            },
                                            _stateSpinBox
                    ...
                    new StripeBack
                    {
                        Children =
                        {
                            new Label
                            {
                                Margin = new(4, 4, 0, 4),
                                Text = Loc.GetString(
                                    "particle-accelerator-control-menu-check-containment-field-warning"),
                                HorizontalAlignment = HAlignment.Center,
                                StyleClasses = { StyleBase.StyleClassLabelSubText, }
                            }
                        }
        ...
        PASegmentControl Segment(string name) => new(this, resourceCache, name);

        UpdateUI(false, false, false, false);
    }

    private bool StrengthSpinBoxValid(int n) => n >= 0 && n <= _maxStrength && !_blockSpinBox;
    ...
    protected override void FrameUpdate(FrameEventArgs args)
    {
        ...
        _drawLabel.Text = Loc.GetString(
            "particle-accelerator-control-menu-draw",
            ("watts", $"{watts:##,##0}"),
    ...
    private sealed class PASegmentControl : Control
    {
        ...
        public PASegmentControl(ParticleAcceleratorControlMenu menu, IResourceCache cache, string name)
        {
            ...
            AddChild(_base = new() { Texture = _rsi["completed"].Frame0, });
            AddChild(_unlit = new());
            MinSize = _rsi.Size;
        }
