        ...
        comp.EmaggingAnimation = new Animation
        {
            ...
            AnimationTracks =
            {
                new AnimationTrackSpriteFlick
                {
                    ...
                    KeyFrames = { new(comp.EmaggingSpriteState, 0f), }
                }
            }
        };
    }
    ...
    private void OnAppearanceChange(EntityUid uid, DoorComponent comp, ref AppearanceChangeEvent args)
    {
        ...
        if (AppearanceSystem.TryGetData<string>(uid, DoorVisuals.BaseRSI, out var baseRsi, args.Component))
        {
            if (!_resourceCache.TryGetResource<RSIResource>(
                SpriteSpecifierSerializer.TextureRoot / baseRsi,
                out var res))
        ...
        switch (state)
        {
            ...
            case DoorState.Opening:
                if (animPlayer != null && comp.OpeningAnimationTime != 0.0)
                    _animationSystem.Play(
                        (uid, animPlayer),
                        (Animation) comp.OpeningAnimation,
                        DoorComponent.AnimationKey);
                break;
            case DoorState.Closing:
                if (animPlayer != null && comp.ClosingAnimationTime != 0.0 && comp.CurrentlyCrushing.Count == 0)
                    _animationSystem.Play(
                        (uid, animPlayer),
                        (Animation) comp.ClosingAnimation,
                        DoorComponent.AnimationKey);
                break;
            case DoorState.Denying:
                if (animPlayer != null)
                    _animationSystem.Play(
                        (uid, animPlayer),
                        (Animation) comp.DenyingAnimation,
                        DoorComponent.AnimationKey);
                break;
            ...
            case DoorState.Emagging:
                if (animPlayer != null)
                    _animationSystem.Play(
                        (uid, animPlayer),
                        (Animation) comp.EmaggingAnimation,
                        DoorComponent.AnimationKey);
                break;
            default:
