...
public sealed class CocoonSystem : EntitySystem
{
    ...
    private void OnCocEntInserted(EntityUid uid, CocoonComponent component, EntInsertedIntoContainerMessage args)
    {
        ...
        if (TryComp<HumanoidAppearanceComponent>(
            args.Entity,
            out var humanoidAppearance)) // If humanoid, use height and width
            cocoonSprite.Scale = new(humanoidAppearance.Width, humanoidAppearance.Height);
        else if (!TryComp<SpriteComponent>(args.Entity, out var entSprite))
            return;
        else if (
            entSprite.BaseRSI !=
            null) // Set scale based on sprite scale + sprite dimensions. Ideally we would somehow get a bounding box from the sprite size not including transparent pixels, but FUCK figuring that out.
