...
public sealed class ConstructionSystem : SharedConstructionSystem
{
    ...
    private void UpdateCraftingAvailability(bool available)
    {
        ...
        CraftingAvailabilityChanged?.Invoke(this, new(available));
    ...
    )
    {
        ...
        sprite.Color = new(48, 255, 48, 128);

        for (var i = 0; i < prototype.Layers.Count; i++)
        {
            sprite.AddBlankLayer(
                i); // There is no way to actually check if this already exists, so we blindly insert a new one
