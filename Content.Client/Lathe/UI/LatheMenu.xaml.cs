...
public sealed partial class LatheMenu : DefaultWindow
{
    ...
    public void PopulateRecipes()
    {
        ...
        foreach (var prototype in sortedRecipesToShow)
        {
            ...
            if (_prototypeManager.TryIndex(prototype.Result, out EntityPrototype? entityProto) && entityProto != null)
                textures = SpriteComponent.GetPrototypeTextures(entityProto, _resources)
                    .Select(o => o.Default)
                    .ToList();
            else
            {
                textures = prototype.Icon == null
                    ? new() { _spriteSystem.GetPrototypeIcon(prototype.Result).Default, }
    ...
    private string GenerateTooltipText(LatheRecipePrototype prototype)
    {
        ...
        foreach (var (id, amount) in prototype.RequiredMaterials)
        {
            ...
            var adjustedAmount = SharedLatheSystem.AdjustMaterial(
                amount,
                prototype.ApplyMaterialDiscount,
                _entityManager.GetComponent<LatheComponent>(_owner).MaterialUseMultiplier);
