...
public sealed class DamageVisualsSystem : VisualizerSystem<DamageVisualsComponent>
{
    ...
    private void HandleDamage(EntityUid uid, AppearanceComponent component, DamageVisualsComponent damageVisComp)
    {
        ...
        if (!AppearanceSystem.TryGetData<DamageVisualizerGroupData>(
            uid,
            DamageVisualizerKeys.DamageUpdateGroups,
            out var data,
            component))
            data = new(Comp<DamageableComponent>(uid).DamagePerGroup.Keys.ToList());
    ...
    )
    {
        if (damageVisComp.DamageOverlayGroups != null)
            UpdateDamageVisuals(
                damageVisComp.DamageOverlayGroups.Keys.ToList(),
                damageComponent,
                spriteComponent,
                damageVisComp);
        else if (damageVisComp.DamageGroup != null)
            UpdateDamageVisuals(new() { damageVisComp.DamageGroup, }, damageComponent, spriteComponent, damageVisComp);
        else if (damageVisComp.DamageOverlay != null)
            UpdateDamageVisuals(damageComponent, spriteComponent, damageVisComp);
    }
