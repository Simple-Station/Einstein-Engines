...
public sealed partial class FancyTree : Control
{
    ...
    public TreeItem? SelectedItem => SelectedIndex == null ? null : Client.Items[SelectedIndex.Value];

    /// <summary>
    ///     If true, a collapsed item will automatically expand when first selected. If false, it has to be manually expanded
    ///     by
    ...
    private void LoadIcons()
    {
        ...
        foreach (var item in Client.Body.Children)
        {
    ...
    public TreeItem AddItem(TreeItem? parent = null)
    {
        if (parent != null)
        {
            ...
            DebugTools.Assert(Client.Items[parent.Index] == parent);
        }

        var item = new TreeItem()
        {
            Tree = this,
            Index = Client.Items.Count
        };
    ...
    {
        if (value == null || value < 0 || value >= Client.Items.Count)
            value = null;
    ...
    {
        Control? current = Client.Items[index];
        while (current != null)
        {
