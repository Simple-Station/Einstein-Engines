...
public sealed class PoweredLightVisualizerSystem : VisualizerSystem<PoweredLightVisualsComponent>
{
    ...
    )
    {
        ...
        if (!AppearanceSystem.TryGetData<PoweredLightState>(
            uid,
    ...
    private void OnAnimationCompleted(EntityUid uid, PoweredLightVisualsComponent comp, AnimationCompletedEvent args)
    {
        ...
        AnimationSystem.Play(
            uid,
            Comp<AnimationPlayerComponent>(uid),
    ...
    private void SetBlinkingAnimation(EntityUid uid, bool shouldBeBlinking, PoweredLightVisualsComponent comp)
    {
        ...
        if (shouldBeBlinking)
            AnimationSystem.Play(
                ...
                BlinkingAnimation(comp),
                PoweredLightVisualsComponent.BlinkingAnimationKey);
        else if (AnimationSystem.HasRunningAnimation(
            uid,
            animationPlayer,
            PoweredLightVisualsComponent.BlinkingAnimationKey))
        ...
        var blinkingAnim = new Animation
        {
            ...
            AnimationTracks =
            {
                ...
                new AnimationTrackSpriteFlick
                {
                    ...
                    KeyFrames =
                    {
                        new(comp.SpriteStateMap[PoweredLightState.Off], 0),
                        new(comp.SpriteStateMap[PoweredLightState.On], 0.5f)
                    }
                }
            }
        };
