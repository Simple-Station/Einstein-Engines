...
public abstract partial class LightBehaviourAnimationTrack : AnimationTrackProperty
{
    ...
    [DataField("startValue")] public float StartValue { get; set; }
...
public sealed partial class LightBehaviourComponent : SharedLightBehaviourComponent, ISerializationHooks
{
    ...
    [ViewVariables(VVAccess.ReadOnly), DataField("behaviours"),]
    ...
    [ViewVariables(VVAccess.ReadOnly)]
    private readonly Dictionary<string, object> _originalPropertyValues = new();
    ...
    void ISerializationHooks.AfterDeserialization()
    {
        ...
        foreach (var behaviour in Behaviours)
        {
            ...
            Animations.Add(new(key, animation, behaviour));
            key++;
        }
    }
    ...
    private void CopyLightSettings(EntityUid uid, string property)
    {
        if (_entMan.TryGetComponent(uid, out PointLightComponent? light))
        {
            ...
            Logger.Warning(
                $"{_entMan.GetComponent<MetaDataComponent>(uid).EntityName} has a {nameof(LightBehaviourComponent)} but it has no {nameof(PointLightComponent)}! Check the prototype!");
    }
