...
public sealed partial class AccessOverriderWindow : DefaultWindow
{
    ...
    public AccessOverriderWindow(
        AccessOverriderBoundUserInterface owner,
    ...
    public void UpdateState(AccessOverriderBoundUserInterfaceState state)
    {
        ...
        if (state.MissingPrivilegesList != null && Enumerable.Any(state.MissingPrivilegesList))
        {
            ...
            foreach (string tag in state.MissingPrivilegesList)
            {
                var privilege = Loc.GetString(
                    _prototypeManager.Index<AccessLevelPrototype>(tag)?.Name ?? "generic-unknown");
    ...
    private void SubmitData() =>
        _owner.SubmitData(
            ...
            Enumerable.ToList(
                Enumerable.Select(
                    Enumerable.Where(_accessButtons, x => x.Value.Pressed),
                    x => new ProtoId<AccessLevelPrototype>(x.Key))));
}
