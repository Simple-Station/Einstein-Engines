...
public sealed class GasCanisterBoundUserInterface : BoundUserInterface
{
    ...
    private void OnTankEjectPressed() => SendMessage(new GasCanisterHoldingTankEjectMessage());

    private void OnReleasePressureSet(float value) => SendMessage(new GasCanisterChangeReleasePressureMessage(value));

    private void OnReleaseValveOpenPressed() => SendMessage(new GasCanisterChangeReleaseValveMessage(true));

    private void OnReleaseValveClosePressed() => SendMessage(new GasCanisterChangeReleaseValveMessage(false));
