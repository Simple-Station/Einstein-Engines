...
public sealed partial class GuideFoodSource : BoxContainer, ISearchableControl
{
    ...
    private void GenerateControl(FoodButcheringData entry)
    {
        ...
        {
            SourceLabel.Text = Loc.GetString("guidebook-food-unknown-proto", ("id", entry.Butchered));
            return;
        }

        SetSource(ent);
        ProcessingLabel.Text = Loc.GetString("guidebook-food-processing-butchering");
        ProcessingTexture.TexturePath = "/Textures/Objects/Weapons/Melee/kitchen_knife.rsi/icon.png"; // hardcoded because eugh
    }
    ...
    private void GenerateControl(FoodSlicingData entry)
    {
        ...
        {
            SourceLabel.Text = Loc.GetString("guidebook-food-unknown-proto", ("id", entry.Sliced));
            return;
        }

        SetSource(ent);
        ProcessingLabel.Text = Loc.GetString("guidebook-food-processing-slicing");
        ProcessingTexture.TexturePath = "/Textures/Objects/Misc/utensils.rsi/plastic_knife.png";
    }
    ...
    private void GenerateControl(FoodRecipeData entry)
    {
        ...
        {
            SourceLabel.Text = Loc.GetString("guidebook-food-unknown-proto", ("id", entry.Result));
            return;
        }
        ...
        SourceLabel.Text = Loc.GetString("guidebook-food-processing-recipe", ("ingredients", combinedIngredients));

        ProcessingTexture.TexturePath = "/Textures/Structures/Machines/microwave.rsi/mwo.png";
    ...
    private void GenerateControl(FoodReactionData entry)
    {
        ...
        {
            SourceLabel.Text = Loc.GetString("guidebook-food-unknown-proto", ("id", entry.Reaction));
            return;
        }
    ...
    private void GenerateOutputs(EntityPrototype result, FoodSourceData entry)
    {
        ...
        if (TryGetIcon(result, out var icon))
            OutputsTexture.Texture = icon;
        else
            OutputsTexture.Visible = false;
    }

    private bool TryGetIcon(EntityPrototype ent, out Texture? texture)
    {
        if (ent.TryGetComponent<SpriteComponent>(out var sprite)
            && sprite.LayerMapTryGet("icon", out var iconIdx)
            && sprite.TryGetLayer(iconIdx, out var icon))
        {
    ...
    {
        SourceLabel.Text = ent.Name;

        if (TryGetIcon(ent, out var icon))
            SourceTexture.Texture = icon;
        else
            SourceTexture.Visible = false;
    }
