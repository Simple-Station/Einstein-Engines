...
public sealed partial class HumanoidProfileEditor : BoxContainer
{
    ...
    private readonly Dictionary<Button, ConfirmationData> _confirmationData = new();
    private readonly List<TraitPreferenceSelector> _traitPreferences = new();
        ...
        foreach (var department in departments)
        {
            ...
            if (!_jobCategories.TryGetValue(department.ID, out var category))
            {
                ...
                if (firstCategory)
                    ...
                    category.AddChild(new() { MinSize = new(0, 23), });
        ...
        switch (skin)
        {
            ...
            case HumanoidSkinColor.TintedHuesSkin: // DeltaV - Tone blending
            {
                ...
                var color = skin switch // DeltaV - Tone blending
                {
                    ...
                    HumanoidSkinColor.TintedHuesSkin => SkinColor.TintedHuesSkin(
    ...
    private void UpdateCustomSpecieNameEdit()
    {
        ...
        _customspecienameEdit.Text = string.IsNullOrEmpty(Profile?.Customspeciename)
    ...
    private void UpdateCMarkingsHair()
    {
            ...
            _markingManager.Markings.TryGetValue(Profile.Appearance.HairStyleId, out var hairProto))
        {
            ...
            {
                hairColor = _markingManager.MustMatchSkin(
                    Profile.Species,
    ...
    private void UpdateCMarkingsFacialHair()
    {
            ...
            _markingManager.Markings.TryGetValue(Profile.Appearance.FacialHairStyleId, out var facialHairProto))
        {
            ...
            {
                facialHairColor = _markingManager.MustMatchSkin(
                    Profile.Species,
                    HumanoidVisualLayers.Hair,
                    out _,
                    _prototypeManager)
    ...
    private async void ExportProfile()
    {
        ...
        StartExport();
        var file = await _dialogManager.SaveFile(new(new FileDialogFilters.Group("yml")));
    ...
    private void UpdateTraitPreferences()
    {
        ...
        TraitPointsLabel.Text = Loc.GetString(
        ...
        TraitsRemoveUnusableButton.Text = Loc.GetString(
            "humanoid-profile-editor-traits-remove-unusable-button",
            ("count", _traits
                .Where(
                    t => _traitPreferences
                        .Where(tps => tps.Preference)
                        .Select(tps => tps.Trait)
                        .Contains(t.Key))
                .Count(t => !t.Value)));
    ...
    private readonly Dictionary<TraitPrototype, bool> _traits = new();
    ...
    public void UpdateTraits(bool? showUnusable = null, bool reload = false)
    {
        ...
        var points = _cfgManager.GetCVar(CCVars.GameTraitsDefaultPoints);
        TraitPointsLabel.Text = Loc.GetString(
            "humanoid-profile-editor-traits-points-label",
            ("points", points),
            ("max", points));
        ...
        {
            TraitsTabs.AddTab(
                new Label { Text = Loc.GetString("humanoid-profile-editor-traits-no-traits"), },
        ...
        var cats = CreateTree(
            _prototypeManager.EnumeratePrototypes<TraitCategoryPrototype>()
                .Where(c => c.Root)
        ...
        foreach (var tab in cats.Select(category => FindCategory(category.ID, TraitsTabs)))
        {
            ...
            if (tab != null)
                ((NeoTabContainer) tab.Parent!.Parent!.Parent!.Parent!).SetTabVisible(
                    tab,
                    tab.Children.First().Children.First().Children.Any());

            // If it has a parent tab container, hide it if it's empty
            if (tab?.Parent?.Parent is NeoTabContainer parent)
            {
                var parentCats = parent.Contents.Select(c => _prototypeManager.Index<TraitCategoryPrototype>(c.Name!))
                    .ToList();
    ...
    private void UpdateLoadoutPreferences()
    {
        ...
        LoadoutPointsLabel.Text = Loc.GetString(
            "humanoid-profile-editor-loadouts-points-label",
            ("points", points),
            ("max", points));
    ...
    public void UpdateLoadouts(bool? showUnusable = null, bool reload = false)
    {
        ...
        // Create a Dictionary/tree of categories and subcategories
        var cats = CreateTree(
            _prototypeManager.EnumeratePrototypes<LoadoutCategoryPrototype>()
                .Where(c => c.Root)
        ...
        void UpdateSelector(LoadoutPreferenceSelector selector, bool usable)
        {
            ...
            foreach (var item in selector.Loadout.Items)
            {
                if (_dummyLoadouts.TryGetValue(
                        selector.Loadout.ID + selector.Loadout.Items.IndexOf(item),
                        out var entity)
    ...
    private void TryRemoveUnusableLoadouts()
    {
        ...
        foreach (var (loadout, _) in
            _loadouts.Where(
                    l =>
                        !l.Value || !_loadoutPreferences.First(lps => lps.Loadout.ID == l.Key.ID).Wearable)
                .ToList())
