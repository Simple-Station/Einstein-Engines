...
public sealed class NodeVisualizationOverlay : Overlay
{
    ...
    private void DrawWorld(in OverlayDrawArgs overlayDrawArgs)
    {
        ...
        var cursorBox = Box2.CenteredAround(_mouseWorldPos, new(nodeSize, nodeSize));
        ...
        foreach (var (gridId, gridDict) in _gridIndex)
        {
            ...
            var (_, _, worldMatrix, invMatrix) = _entityManager.GetComponent<TransformComponent>(gridId)
            ...
            foreach (var (pos, list) in gridDict)
            {
                ...
                foreach (var (group, node) in list)
                {
                    ...
                    _nodeIndex[(group.NetId, node.NetId)] = new(group, node, nodePos);
                    offset += nodeOffset;
                }
            }
            ...
            foreach (var nodeRenderData in _nodeIndex.Values)
            {
                ...
                var bounds = Box2.CenteredAround(pos, new(nodeSize, nodeSize));
