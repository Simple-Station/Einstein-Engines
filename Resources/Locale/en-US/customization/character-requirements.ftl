character-age-requirement = You must be within {$min} and {$max} years old
character-species-requirement = You must be a {$species}
character-trait-requirement = You must have the trait {$trait}
character-backpack-type-requirement = You must use a {$type} as your bag
character-clothing-preference-requirement = You must wear a {$type}

character-job-requirement = You must be one of these jobs: {$job}
