...
public sealed class BloodSuckerSystem : EntitySystem
{
    ...
    private void OnDamageChanged(EntityUid uid, BloodSuckedComponent component, DamageChangedEvent args)
    {
        if (args.DamageIncreased
            || !_prototypeManager.TryIndex<DamageGroupPrototype>("Brute", out var brute)
    ...
    public bool TrySucc(EntityUid bloodsucker, EntityUid victim, BloodSuckerComponent? bloodsuckerComp = null)
    {
        ...
        if (!Resolve(bloodsucker, ref bloodsuckerComp)
            ...
            || _bloodstreamSystem.GetBloodLevelPercentage(victim, bloodstream) != 0.0f
            || !_bodySystem.TryGetBodyOrganComponents<StomachComponent>(bloodsucker, out var stomachList)
            || stomachList.Count == 0
            || !_solutionSystem.TryGetSolution(stomachList[0].Comp.Owner, StomachSystem.DefaultSolutionName, out var _))
            return false;
